...
int main(void){
    ...
    if(mysql_real_connect(my_conn,"127.0.0.1","root","root","CasDB",3306,NULL,0)){
        FILE *fout=fopen("/var/lib/mysql/CasDB/out.sql","w");
        char path[]="../Database/";
        strcpy(buffer,path);
        ...
        int res2=mysql_query(my_conn,"load data infile 'out.sql' ignore into table Table_sgRNA fields terminated by ',' enclosed by '\"' lines terminated by '\n' (Chr_No, sgrna_start, sgrna_end, sgrna_strand, sgrna_seq, sgrna_PAM);");
